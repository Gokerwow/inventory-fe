export default function DropdownInput({ open, options, placeholder }: { open: boolean; options: string[], placeholder?: string }) {
    const [open, setOpenPihakPertama] = useState(false);
    const [openPihakKedua, setOpenPihakKedua] = useState(false);

    return (
        <div>
            <button
                onClick={() => setOpenPihakPertama(!open)}
