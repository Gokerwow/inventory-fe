...
export default function PenerimaanTable({
    ...
    const penerimaanColumns: ColumnDefinition<PenerimaanItem>[] = [
        ...
        },
        {
            header: 'AKSI',
            cell: (item) => (
                <div onClick={() => handleActionClick(item.id)} className='flex items-center gap-1 w-20 cursor-pointer hover:scale-110 active:scale-95 transition-all duration-200 hover:text-blue-400'>
                    {user?.role === 'Tim PPK' ?
                        <>
