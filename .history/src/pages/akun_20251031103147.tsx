...
export default function MonitoringPage() {
    ...
    const handleEditClick = (userData) => {
        navigate(PATHS.PROFIL.EDIT, {
            state: {
                data: userData, // atau data lain yang ingin dikirim
                // contoh:
                // id: user.id,
                // name: user.name,
                // email: user.email
    ...
    useEffect(() => {
        // Jangan check access selama loading
        if (!loading) {
            checkAccess(user?.role);
        }
    }, [user, checkAccess, loading]);

    // Tampilkan loading spinner
    if (loading) {
        return (
            <div className="min-h-full p-8 bg-[#F3F7FA] rounded-lg shadow-md flex items-center justify-center">
