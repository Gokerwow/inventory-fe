...
export default function TambahPenerimaan({ isEdit = false }: { isEdit?: boolean }) {
    ...
    const [selectedFirstNama, setSelectedFirstNama] = useState("");
    const [selectedSecondNama, setSelectedSecondNama] = useState("");

    // CONTOH
    const [formDataPenerimaan, setFormDataPenerimaan] = useState({
        id: 0,
        noSurat: '',
        namaPihakPertama: '',
        jabatanPihakPertama: '',
    ...
    const handleAddClick = (barang: TIPE_BARANG_BELANJA) => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        const target = e.target as (HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement) & { name: keyof FormData };
        const { name, value } = target;
        setFormDataPenerimaan((prevState: FormData) => ({
            ...prevState,
            [name]: value,
        }));
    };

    // const handleActionClick = ({ type } : { type: string }) => {
    //     forkm
    // }

    const handleSubmit = (e: React.ChangeEvent<HTMLFormElement>) => {
        e.preventDefault();
        console.log('Form Data Penerimaan:', formDataPenerimaan);
    }
                ...
                <div className='grid grid-cols-2 gap-8'>
                    ...
                    <div className="flex flex-col gap-6 p-6 bg-white rounded-2xl shadow-[0_0_10px_rgba(0,0,0,0.1)]">
                        ...
                        <DropdownInput
                            ...
                            judul='Nama Lengkap'
                            value={selectedFirstNama}
                                value={formDataPenerimaan.namaPihakPertama}  // ← Langsung dari form state
    onChange={(value) => {
        setFormDataPenerimaan(prev => ({
            ...prev,
            namaPihakPertama: value  // ← Langsung update form state
        }))
    }}
                    ...
                    <div className="flex flex-col gap-6 p-6 bg-white rounded-2xl shadow-[0_0_10px_rgba(0,0,0,0.1)]">
                        ...
                        <DropdownInput
                            ...
                            judul='Nama Lengkap'
                            type='button'
                            value={selectedSecondNama}
                            onChange={setSelectedSecondNama}
                            name='namaPihakKedua'
                        />
                    ...
                    {!barang || barang.length === 0 ?
                        ...
                        <>
                            <div className="flex justify-between items-center border-b border-gray-200 p-6">
                                ...
                                <button onClick={handleAddClick} className="bg-green-500 cursor-pointer hover:scale-110 active:scale-95 transition-all duration-200 rounded-lg text-white font-medium py-2 px-5 shadow-sm">
                                    Tambah Barang
                                </button>
                            </div>
