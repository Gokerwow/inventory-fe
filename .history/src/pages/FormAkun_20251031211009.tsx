...
export default function TambahAkunPage({ isEdit }: { isEdit?: boolean }) {
 ...
 const [formData, setFormData] = useState({
  ...
  useEffect(() => {
    ...
    setFormData(prev => ({
      ...prev,
      user_id: prev.user_id || Date.now(), // Simple ID generator
      created_at: prev.created_at || currentTimestamp,
      updated_at: currentTimestamp,
      last_login: currentTimestamp,
      role: prev.role || 'user',
      avatarUrl: prev.avatarUrl || '/default-avatar.png',
      sso_user_id: 0, // Default value
    }));
  }, []);
