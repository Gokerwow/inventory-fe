    ...
    return (
        <div className="min-h-full p-8 bg-[#F3F7FA] rounded-lg shadow-md">
            <div className="relative mb-8" ref={dropdownRef}>
                ...
                <button onClick={() => setIsOpen(!open)} aria-label='Menu' className="absolute top-0 right-0 p-3 bg-white rounded-lg shadow-md hover:bg-gray-100 cursor-pointer  hover:scale-110 active:scale-95 transition-all duration-200">
                    <BurgerDot className='w-10 h-10' />
                </button>

                {/* Dropdown Menu */}
                {isOpen && (
                    <div className="absolute right-0 z-50 w-56 mt-2 origin-top-right bg-white border border-gray-200 rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                        {/* Header */}
                        ...
                        <div className="py-2">
                            {sortOptions.map((option) => (
                                <button
                                    ...
                                    className={`flex items-center w-full px-4 py-2.5 text-sm transition-colors duration-150 ${selectedSort === option.label
