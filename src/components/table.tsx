...
// 1. CHANGE: Removed "extends { id?: number | string }"
// Now T can be ANYTHING.
            ...
            <div className={`md:hidden flex-1 overflow-y-auto p-4 space-y-4 ${mobileContainerClassName || 'pb-8 sm:pb-4'}`}>
                {currentItems && currentItems.length > 0 ? (
                    currentItems.map((item, index) => (
                        <div 
                            // 2. CHANGE: Added (item as any) to allow checking for 'id' on generic T
                            // Falls back to index if 'id' doesn't exist.
                        ...
                        >
                            {columns.map((col, colIndex) => (
                                <div key={col.key || colIndex} className="flex justify-between items-center gap-4 text-sm border-b border-gray-50 last:border-0 pb-2 last:pb-0">
                                    ...
                                    <div className={`font-semibold text-gray-800 flex-1 min-w-0 wrap-break-word text-left ${!col.hideHeaderOnMobile ? '' : 'w-full text-center'}`}>
                                        {col.cell(item)}
                                    </div>
                                </div>
                            ))}
            ...
            <div className="hidden md:flex flex-col flex-1 overflow-hidden">
                <div className="flex-1 overflow-y-auto">
                    ...
                    {currentItems && currentItems.length > 0 ? (
                        currentItems.map((item, index) => (
                            <div
                                // 3. CHANGE: Same here, use (item as any)
                                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                key={(item as any).id || index}
                                onClick={() => onRowClick && onRowClick(item)}
